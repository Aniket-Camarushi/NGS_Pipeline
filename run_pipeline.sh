...
echo "Running GATK variant calling..."
mkdir -p GATK

# First create a sequence dictionary for the reference
...
# Combine GVCFs (FIXED: use a loop to include all samples)
GVCFs=$(ls ./GATK/*.g.vcf.gz | tr '\n' ' ')  # Get all GVCFs
gatk CombineGVCFs \
    -R ./Data/Homo_sapiens.GRCh38.dna.primary_assembly.fa \
    $(printf -- "--variant %s " $GVCFs) \  # Dynamically add all samples
...
# 7. HARD FILTERING (instead of VQSR)
